...
`setup-api` for plugins that still need setup-time runtime hooks. Setup-time
service checks can also use `setup.services` after the owning plugin is already
known. If more than one discovered plugin claims the same normalized setup
provider or CLI backend id, setup lookup refuses the ambiguous owner instead of
